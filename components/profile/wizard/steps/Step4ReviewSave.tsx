...
export default function Step4ReviewSave({ data, onBack }: Props) {
  ...
  /** ----------------------------------------
   * SAVE HANDLER (CONNECTED TO BACKEND)
   * ---------------------------------------*/
      ...
      {status === "idle" && (
        <>
          <div className="space-y-4 text-sm">
            ...
            <ReviewItem label="Languages" value={data.languages?.join(", ")} />
            <ReviewItem label="Level" value={data.level} />
            <ReviewItem label="ID / Passport" value={data.idNumber} />
            <ReviewItem label="TRA Number" value={data.traNumber} />
            <ReviewItem label="License Number" value={data.licenseNumber} />
            <ReviewItem
              label="Experience"
          ...
          <div className="pt-24 flex justify-between">
            ...
            <button
              onClick={handleSave}
              className="px-10 py-4 rounded-full bg-green-600 text-white text-sm font-medium"
            >
              SAVE PROFILE
            </button>
          </div>
