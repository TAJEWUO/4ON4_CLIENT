...
export default function VerifyOtpForm() {
  ...
  // Load email + mode from localStorage
  useEffect(() => {
    const storedEmail = localStorage.getItem("fouron4_auth_email");
    const storedMode =
      (localStorage.getItem("fouron4_verify_mode") as "register" | "reset") ||
      "register";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email) {
      setMsg("Missing email context. Start again from Register / Forgot PIN.");
      return;
    }
    ...
    if (mode === "register") {
      const { ok, data } = await postAuth("/api/auth/register-verify", {
        email,
        otp: code,
      });
      setLoading(false);
      ...
      // mode === "reset"
      const { ok, data } = await postAuth("/api/auth/forgot-pin-verify", {
        email,
        otp: code,
      });
      setLoading(false);
      ...
      // Expect backend to return phone linked to email + reset token
      if (data?.phone) {
      ...
      localStorage.setItem("fouron4_reset_email", email);
